...
class mdplugin():
    ...
    async def on_player_command(self, player: str, message: str):
        ...
        if not player in self.server.admins:
            ...
            self.server.show_command(player, f"adreload", "Recargar los mdaddons.")
        ...
        elif self.server.is_command(message, 'adreload'):
            await self.server.client._load_addons(reload = True)
            self.server.send_response(player, "✔ mdaddons recargados.")
